...
function mColor(mc) {
	...
	this.getHex = function () {
		return (
			'#' +
			(0x100 | Math.round(this.rgb.r)).toString(16).substring(1) +
			(0x100 | Math.round(this.rgb.g)).toString(16).substring(1) +
			(0x100 | Math.round(this.rgb.b)).toString(16).substring(1)
		);
	};
...
function importRGB(oa, validate) {
	...
	function hexToDec(hex) {
		return sanitizeRGB(parseInt(hex, 16), validate);
	}
...
function XYZtoSRGB(xyz) {
	...
	srgb.b = xyz.x * 0.0557 + xyz.y * -0.204 + xyz.z * 1.057;
